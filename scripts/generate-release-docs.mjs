...
function normalizeReleaseMetadata(raw) {
  ...
  return {
    ...
    summary,
    stats: {
      added: toNumberOrZero(stats.added),
      changed: toNumberOrZero(stats.changed),
      fixed: toNumberOrZero(stats.fixed),
      actionRequired: toNumberOrZero(stats.actionRequired),
    },
    releaseNotesPath,
  };
}
...
function buildAutoChangelogPage({ releasesList, metadataList, slug = "/changelog" }) {
  ...
  return [
    ...
    `    <span class="changelog-chip">Added: ${latest.stats.added}</span>`,
    `    <span class="changelog-chip">Changed: ${latest.stats.changed}</span>`,
    `    <span class="changelog-chip">Fixed: ${latest.stats.fixed}</span>`,
    `    <span class="changelog-chip chip-warn">Action required: ${latest.stats.actionRequired}</span>`,
    "  </div>",
    "</div>",
    "",
